...
citeproc: true
abstract: |
  A four-letter word is doing a lot of work in English; it's the polite one. Speakers omit *that*
  ...
  linguistic knowledge includes not just what's possible, but what's probable.
---
...
**The central claim of this paper:** English linguistic knowledge includes not just what's possible, but what's probable. Alternation probabilities are linguistic knowledge.

Specifically, they are **morphosyntactic knowledge**. I use [morphosyntactic]{.smallcaps} rather than "grammatical" to avoid ambiguity. In linguistics, *grammatical* is a three-way ambiguity in a trench coat: it sometimes means "well-formed" (acceptability), sometimes "morphosyntactic" (as opposed to phonological or lexical), and sometimes "part of linguistic competence." The claim here is that probabilistic preferences among well-formed structural alternatives are morphosyntactic knowledge -- systematic, shared, and predictive -- even though violating them doesn't trigger sharp unacceptability.
...
systematically characterize the conditioning factors. Reference grammars are excellent maps; they're just not traffic reports. They describe what's possible, not what's probable. The
present analysis takes the next step, asking what determines the probability of
each variant.

### Functions of content clauses

Content clauses serve several syntactic functions, and the availability of the
...
What makes this "morphosyntactic knowledge" rather than purely pragmatic or stylistic variation? The claim is not just that probabilistic mappings exist in the data, but that speakers internalize them as part of their competence. Register, in this view, is not a contaminant to be filtered out but part of the input ecology. Speakers learn conditioned expectations over register just as they learn them over lexical items.

I argue that the alternation is morphosyntactic insofar as it is *manipulable* and *projectible*. It is manipulable in a causal sense: if we intervene to lengthen a clause or extrapose it, the probability of *that* shifts in a predictable direction. It is projectible in the sense that the pattern recurs across speakers and settings, supporting reliable prediction to new cases [@Goodman1955]. The question isn't whether to draw a line between morphosyntax and pragmatics, but whether this phenomenon belongs to a stable node in the causal network that constituted linguistic knowledge.

There's a further argument that the alternation is grammatical even in the acceptability-judgment sense. Complement selection is paradigmatically grammatical: *want* takes infinitival complements, not finite (*I want to leave* vs. *\*I want that I leave*); *wonder* takes interrogative complements, not declarative. The *that* ~ bare alternation is a case of complement selection&nbsp;-- choice between a marked and unmarked clause type. And in subject function, the choice *is* categorical: *\*She left surprised everyone* is sharply degraded. The same is true of *whether* vs. *if*, which alternate freely in some environments (*I wonder whether/if she left*) but not others (*the question of whether/\*if she left*). When does complement selection stop being grammatical? It doesn't. It becomes gradient.
...
1. **Lemma-level entrenchment**: Individual verbs develop baseline omission rates
   ...
   Frequency drives entrenchment. Entrenchment shapes preference. Preference becomes knowledge.
...
@tbl-desc-head breaks down the data by head category and syntactic function.

| Head Category | Function | N | that-rate (%) |
| :--- | :--- | :---: | :---: |
| Verb | Complement | 230,058 | 24.3 |
| | Extraposed | 7,977 | 26.3 |
| *be* | Complement | 52,953 | 19.8 |
...
To capture extraposed clauses (e.g., *It's clear...*), I identify [ccomp]{.smallcaps} relations where the matrix predicate has an expletive *it* as subject, excluding specificational *it* + *be* constructions (treated as complements)^[Specificational constructions with *it* (e.g., *It is that...*, *It's just that...*) are distinct from extraposition. In the corpus, these cases (N $\approx$ 17,000) show a low *that*-rate (~20%) similar to other *be*-complements. Excluding them reduces the sample size for extraposition but ensures the category reflects the relevant heavy-subject alternation, preventing specificational semantics from confounding the structural heavy-shift effect.]. This yields 8,186 extraposed tokens. Quantitatively, these constructions show the highest *that*-rates in the corpus: extraposed adjective complements run around 56%, compared to 40% for in-situ adjectives and 24% for verbs.
...
The decision to focus on complement and extraposed clauses&nbsp;-- where both *that* and bare variants
genuinely compete&nbsp;-- reflects a principled approach to defining the phenomenon.
Including subject clauses, where *that* is near-obligatory, would be like predicting that stop signs will be red: true, but not illuminating. It would inflate the
...
I fit a Bayesian logistic regression predicting *that*-realization from clause length (subtree size in tokens), head-clause distance (tokens between the matrix predicate and the clause head), and extraposition status. Continuous predictors are standardized (z-scored: mean 0, SD 1). The model has a multilevel structure that reflects the nested nature of the data:

- **Lemma-level random intercepts**: Each lemma (verb, adjective, or noun) gets its own baseline *that*-rate, drawn from a shared population. This captures lexical specificity (e.g., *think* preferring bare, *demonstrate* preferring *that*) while allowing partial pooling across high- and low-frequency items.
- **Register-specific intercepts**: Spoken, journalism, and academic registers have distinct baselines, capturing the large observed differences in *that*-rate. These are estimated as absolute means (log-odds) for an average lemma in an in-situ context.
...
*that*-rate above 80%. The model captures this heterogeneity through the parameter $\sigma_{\text{lemma}}$, which measures the standard deviation of verb intercepts. This value is estimated at **1.77** (90% CI: [1.61, 1.94]) on the log-odds scale, a large value indicating substantial lexical specificity. To put this in perspective: a shift of two standard deviations in verb identity (e.g., moving from an average verb to a strongly *that*-preferring one) changes the predicted probability more than the entire effect of register.
...
The model estimates register intercepts (on the log-odds scale) of **−2.70** (90% CI: [−2.87, −2.52]) for spoken, **−1.74** (90% CI: [−1.90, −1.57]) for journalism, and **−1.42** (90% CI: [−1.59, −1.24]) for academic. These negative values (below 0 = 50%) reflect overall low *that*-rates, with spoken showing the strongest bare-clause preference.
...
that the patterns are morphosyntactic; it clarifies what *kind* of morphosyntactic
knowledge is involved. Speakers track register as a conditioning context, adjusting their baseline expectations much as they adjust for verb choice.
...
Longer clauses favour *that*. The coefficient for clause length ($\beta_{\text{len}}$) estimates the change in log-odds for a one-standard-deviation increase in length. The model finds $\beta_{\text{len}} =$ **0.57** (90% CI: [0.28, 0.84]). In concrete terms: moving from a short clause (1 SD below mean) to a long clause (1 SD above) shifts the predicted *that*-rate from roughly 20% to 30% in spoken registers, and from 35% to 45% in academic prose. This effect is consistent with processing accounts: *that* may function as a cue that a clause boundary is coming, helping listeners parse complex material.

![Processing effects: Clause length strongly predicts *that* use (Panel A), while distance between head and clause shows a weaker, negative effect (Panel B).](results/figures/effects_combined.png){#fig-effects width="100%" fig-align="center"}

The distance effect is weakly negative ($\beta_{\text{dist}} =$ **−0.53**, 90% CI: [−1.03, 0.11]), meaning that greater distance between the matrix verb and the clause slightly disfavours *that*, but with substantial uncertainty. The data declines to play the role it was cast for. One possibility: high-distance contexts co-occur with parenthetical uses (e.g., "She left early, I think"), where *that* is near-categorical absent. If so, the effect is real but reflects a confounded construction type rather than a pure processing mechanism.

Extraposition shows a modest positive effect ($\beta_{\text{extrap}} =$ **0.30**, 90% CI: [0.06, 0.53]): extraposed clauses favour overt *that* compared to in-situ complements, consistent with the higher *that*-rates observed for adjective-headed extraposed constructions. (The remaining N=106 extraposed tokens not shown in @tbl-desc-head include nouns and adverbs, which are included in the model.)

Processing effects are modest relative to lexical effects. Verb
identity matters more than clause length. Speakers' internalized verb-specific preferences dominate contextual adjustment.
...
For out-of-sample validation, I hold out 20% of documents (stratified by register) and predict their *that*-rates from a model trained on the remaining 80%. The predictions are close to observed rates: for journalism, the model predicts 31.4% (observed 30.0%); for academic, it predicts 41.4% (observed 44.3%). Spoken rates are well aligned (observed 10.6%, predicted 10.0%). The academic register shows the largest discrepancy, with the model slightly underpredicting (see Appendix for by-register figures).
...
settings; it's sensitive to manipulable variables (register practices, clause
...
This doesn't mean traditional categorical syntax is wrong. Many aspects of English are all-or-nothing: *she left* is grammatical, *she leaved* isn't. Categorical constraints define what speakers can't say; gradient preferences define what speakers don't. Alternation phenomena like subordinator realization occupy the gradient part of the landscape&nbsp;-- where probabilities, not just possibilities, define what native speakers know.
...
Third, the complexity effects support @Rohdenburg1996's *Complexity Principle*: longer clauses favour the more explicit variant. The effect size ($\beta_{\text{len}} \approx 0.57$) is modest but robust, suggesting that *that* does function as a parsing cue, as proposed by @BresnanFord2010.
...
If alternation probabilities are morphosyntactic, then grammar isn't purely categorical. Usage-based and constructionist approaches have emphasized this for decades [@Bybee2010; @Goldberg2006], but the present analysis provides rigorous evidence: a quantitative model that captures the phenomenon and validates out of sample.
...
only [ccomp]{.smallcaps} clauses are included. Extending to clausal subjects remains for future work.
...
The choice between *that*-content clauses and bare
...
register, and processing factors. English speakers know more about *that* than they think&nbsp;-- often while omitting it.
...
Future work extends to dative, genitive, particle placement&nbsp;-- testing whether the same factors predict the same patterns. If they do, we'll have evidence that alternations as
a class reflect shared morphosyntactic mechanisms.

## Acknowledgments

This paper was developed with assistance from large language models, including OpenAI's ChatGPT 5.2 Pro, Anthropic's Claude Opus 4.5, and Google's Gemini 3. These tools were used for drafting, critique, code review, and iterative refinement of both prose and statistical analyses. I remain solely responsible for all claims, errors, and interpretations.
...
$$
\text{logit}(P(y_i = 1)) = \alpha + \alpha_{\text{reg}[i]} + \alpha_{\text{lemma}[i]} + (\beta_{\text{len}} + b_{\text{len,reg}[i]}) \cdot \text{len}_i + (\beta_{\text{dist}} + b_{\text{dist,reg}[i]}) \cdot \text{dist}_i + \beta_{\text{extrap}} \cdot \text{extrap}_i
$$
...
- $\beta_{\text{len}}, \beta_{\text{dist}}, \beta_{\text{extrap}}$ are the population-level slopes
...
| :--- | :---: | :---: | :---: | :---: |
| Intercept (Spoken) | -2.70 | [-2.87, -2.52] | 1.02 | 112 |
| Intercept (Journalism) | -1.74 | [-1.90, -1.57] | 1.02 | 101 |
| Intercept (Academic) | -1.42 | [-1.59, -1.24] | 1.02 | 106 |
| Length ($\beta_{\text{len}}$) | 0.57 | [0.28, 0.84] | 1.01 | 297 |
| Distance ($\beta_{\text{dist}}$) | -0.53 | [-1.03, 0.11] | 1.00 | 314 |
| Extraposition ($\beta_{\text{extrap}}$) | 0.30 | [0.06, 0.53] | 1.00 | 1024 |
| $\sigma_{\text{lemma}}$ | 1.77 | [1.61, 1.94] | 1.00 | 91 |
| $\sigma_{\text{len\_reg}}$ | 0.28 | [0.08, 0.77] | 1.01 | 426 |
| $\sigma_{\text{dist\_reg}}$ | 0.57 | [0.20, 1.21] | 1.00 | 499 |
...
The model was fit using CmdStanR with 2 chains of 1000 iterations each (500 warmup). Divergent transitions: 0/1000 at adapt_delta = 0.95. All R-hat values ≤ 1.02. Effective sample sizes > 90 for all parameters.
