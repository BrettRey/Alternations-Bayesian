...
though the patterns may differ from non-extraposed complements. *It's clear that
she left* and *It's clear she left* are both acceptable, but the rate of *that*
may differ from direct complements.

The present analysis focuses on complement content clauses -- the position where
variation is most productive. This follows the variationist principle of
defining an [envelope of variation]{.smallcaps}: we study alternation only where
both variants genuinely compete. Including subject clauses, where *that* is
near-obligatory, would inflate the *that*-rate and obscure the conditioning
factors that govern the choice in variable contexts.
...
Research on *that*-omission has identified several conditioning factors.

First, individual verbs differ sharply. Verbs like *think*, *know*, and *guess*
strongly favour bare clauses; verbs like *prove*, *demonstrate*, and *ensure*
strongly favour overt *that*. This lexical specificity suggests that speakers
learn verb-by-verb patterns, not just a general preference.

Second, register matters. Spoken English uses bare clauses more often than
written English. Within written registers, academic prose uses *that* more than
journalism. These patterns reflect formality, planning time, and genre
conventions.

Third, complexity affects the choice. Longer content clauses favour overt *that*,
as do clauses with intervening material between the matrix verb and the clause
boundary. These effects align with processing accounts: *that* may function as a
parsing cue, signalling "clause boundary coming" and reducing comprehension
difficulty.

Fourth, information-theoretic accounts invoke surprisal. When the content clause
begins with unpredictable material, speakers may insert *that* to spread
information density more evenly -- a prediction of the uniform information
density hypothesis.
...
language system rather than computed on the fly.
...
word order rules. They're facts about English that native speakers know.
...
questions. Technical details appear in the Appendix.

### Why Not Traditional Statistics?

Most linguistic research uses significance tests. You fit a model, get a
*p*-value, and conclude that the effect is "significant" or not. This approach
has served linguistics well, but it has limitations.

First, *p*-values don't tell you what you want to know. A *p*-value says: "If
there were no effect, how surprising would my data be?" But linguists usually
want to ask: "How strong is the effect?" or "How confident should I be that
*think* differs from *prove*?" Bayesian methods answer these questions directly.

Second, significance testing treats each verb (or register, or predictor) in
isolation. But verbs aren't independent: *think* and *believe* might pattern
similarly because they're both cognitive verbs. With sparse data for some verbs,
we'd like to borrow information from similar verbs. Bayesian multilevel models
do this automatically through [partial pooling]{.smallcaps}.

Third, traditional approaches ignore prior knowledge. We know something about
language before we analyze our data: effects bigger than 10 log-odds are
implausible, spoken and written registers differ, and so on. Bayesian methods
let us encode this knowledge and update it with evidence.

### What Bayesian Modeling Gives You

In a Bayesian analysis, the output isn't a single point estimate ("the effect is
0.51") but a [distribution]{.smallcaps} of plausible values. The model says, "Based
on the data and my prior assumptions, the effect is probably between 0.3 and 0.7,
with the most likely value around 0.5." This distribution captures what we've
learned and how uncertain we remain.

For linguistic questions, this is valuable. Instead of asking "Is the register
effect significant?" (yes/no), we can ask "How different are spoken and written
registers?" (continuous) and "How confident are we?" (the spread of the
distribution).

### Partial Pooling for Lexical Effects

Suppose we want to estimate *that*-rates for 500 different verbs. For common
verbs like *think*, we have thousands of tokens, so we can estimate their rates
precisely. For rare verbs like *allege*, we might have only 20 tokens, so our
estimates are noisy.

A naive approach treats each verb independently. But this wastes information:
rare verbs with wildly unusual estimates are probably showing noise, not real
idiosyncrasy. A better approach pools information across verbs, shrinking
extreme estimates toward the overall mean.

Bayesian multilevel models do this through [partial pooling]{.smallcaps}. Common
verbs get estimates close to their observed rates (little shrinkage). Rare verbs
get estimates pulled toward the overall mean (more shrinkage). The amount of
shrinkage is estimated from the data itself, based on how much true variability
exists among verbs.

This is what linguists do intuitively: we assume rare verbs probably behave like
similar verbs unless we have strong evidence otherwise. Bayesian modeling
...
I use the Open American National Corpus (OANC, GrAF release), which includes
spoken and written registers. I parse documents with spaCy and identify finite
content clauses via the Universal Dependencies [ccomp]{.smallcaps} relation.
For each clause, I record whether the subordinator *that* is present (via the
[mark]{.smallcaps} relation with lemma *that*).
...
length (subtree size) and head-clause distance (tokens between matrix verb and
clause head). The model includes:
...
register, do longer clauses favour *that*? Do more distant clauses favour *that*?
...
*that*-rate above 80%. The model captures this heterogeneity: the estimated
standard deviation of verb intercepts (σ~lemma~) is approximately 1.77 on the
log-odds scale, indicating substantial lexical specificity.
...
Longer clauses favour *that*, with an estimated coefficient around β~len~ ≈ 0.51.
This effect is consistent with processing accounts: *that* may function as a cue
that a clause boundary is coming, helping listeners parse complex material.

The distance effect is negative (β~dist~ ≈ −0.50), meaning that greater distance
between the matrix verb and the clause slightly disfavours *that*. This is
