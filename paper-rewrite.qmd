...
Fitting a model is only half the job. We also need to ask: does the model capture the patterns in the data? We answer this using [posterior predictive checks]{.smallcaps}: we use the model to generate fake data and compare it to the real data.

If our model captures *that*-realization well, the fake data should have similar *that*-rates by register, by verb, and overall. If it doesn't, we're missing something important. This emphasis on prediction rather than significance distinguishes Bayesian workflow from traditional hypothesis testing. We're not asking "Is there an effect?" but "Does our model capture the phenomenon?"
