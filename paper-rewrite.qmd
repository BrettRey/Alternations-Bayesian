...
Second, we validate the model using [posterior predictive checks]{.smallcaps}. We ask: "If we simulate new data from our model, does it look like the real data?" If the simulated *that*-rates match the observed ones across registers and verbs, we can trust that the model captures the underlying grammatical system.
