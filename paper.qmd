...
## Model
I fit a Bayesian logistic regression with varying effects for lexical heads and register. Predictors are standardized clause length (subtree size) and head–clause distance (linear token distance between the matrix head and the clause head). The model includes:
...
- Plan prior predictive checks and model expansion once I add new predictors.

## Results
Baseline summary (first pass). The extracted dataset contains 302,522 content-clause tokens with an overall *that*-content clause rate of 0.239. A baseline logistic model with lemma-level partial pooling and register-specific intercepts (plus register-varying length and distance slopes) fit to a 20,000‑token sample yields a positive length effect (β<sub>len</sub> ≈ 0.51) and a negative distance effect (β<sub>dist</sub> ≈ −0.50). Lemma heterogeneity remains large (σ<sub>lemma</sub> ≈ 1.77), while register-specific slope variation is modest (σ<sub>len</sub><sub>reg</sub> ≈ 0.18; σ<sub>dist</sub><sub>reg</sub> ≈ 0.41). Posterior predictive checks on the fit sample track observed overall and by-register rates closely; the current fit has 2/1000 divergent transitions at adapt_delta = 0.95.

Out-of-sample check (document-level holdout, 20% of documents stratified by register). Training uses 20,000 tokens; the held‑out set contains 4,819 tokens after dropping 181 unseen‑lemma tokens. Out-of-sample PPCs slightly underpredict written registers (written 1: obs 0.332 vs pred 0.305; written 2: obs 0.478 vs pred 0.454), while spoken is well aligned (obs 0.106 vs pred 0.106). Overall, the held‑out rate is 0.246 with a predicted median of 0.233; the average held‑out expected log predictive density (ELPD) is −0.477 per token.
